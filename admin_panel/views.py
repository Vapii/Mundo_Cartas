...
from productos.models import Producto, Categoria, CodigoBarras
...
def crear_producto(request):
    ...
    if request.method == "POST":
        ...
        codigos_barras = request.POST.get("codigo_barras")  # puede ser varios separados por coma/salto de línea
        categorias_seleccionadas = request.POST.getlist("categorias")

        if not codigos_barras:
            messages.error(request, "Debes ingresar o escanear al menos un código de barras.")
        ...
        # Crear códigos asociados
        for c in codigos_barras.replace("\n", ",").split(","):
            c = c.strip()
            if c:
                if CodigoBarras.objects.filter(codigo=c).exists():
                    messages.warning(request, f"El código {c} ya existe y no se agregó.")
                else:
                    CodigoBarras.objects.create(producto=producto, codigo=c)
...
def editar_producto(request, id):
    ...
    if request.method == "POST":
        ...
        nuevos_codigos = request.POST.get("nuevos_codigos")
        if nuevos_codigos:
            for c in nuevos_codigos.replace("\n", ",").split(","):
                c = c.strip()
                if c and not CodigoBarras.objects.filter(codigo=c).exists():
                    CodigoBarras.objects.create(producto=producto, codigo=c)
...
def detalle_producto(request, id):
    producto = get_object_or_404(Producto, id=id)
    codigos = producto.codigos.order_by('-id')  # ordenados por creación (si usas auto-incremental)
    return render(request, 'admin_panel/productos/detail_producto.html', {
        "producto": producto,
        "codigos": codigos
    })
